...
const Widget: React.FC<IProps> = ({ baseSite }) => {
  ...
  const onSearchClicked = () => {
    ...
    window.location.href = `${baseURL}/results?${queryString.stringify(searchParams)}`;
  };
      ...
      <div className="flex items-end justify-center">
        <div className="w-1/2 mr-3">
          <input
            ...
            type="text"
            defaultValue={'07040'}
            onChange={(event: any) => onSearchParamsUpdate(searchParams, event.target.value, 'leaseregion', setSearchParams)}
            placeholder="Enter your zip code" />
        </div>
        <div className="w-3/10 mr-3">
          <Dropdown
            options={getCarMakeList()}
            onSelected={(value: string) => onSearchParamsUpdate(searchParams, value, 'carmake', setSearchParams)}
            placeholder="Make"
        ...
        <div className="w-1/5">
          <button
            ...
            disabled={typeof searchParams.carmake === 'undefined' || searchParams.leaseregion.length === 0}
